...
- **Auth**: as of the session/API-key refactor (see [AUTH_SESSIONS_PLAN.md](AUTH_SESSIONS_PLAN.md)), `verifyToken`/`getUserInfo` in [server/middleware/auth.middleware.js](server/middleware/auth.middleware.js) no longer verify JWTs — they read an opaque bearer token (from the `Authorization: Bearer` header or an `access_token` cookie) and resolve it against either the `Session` collection (mobile-app device sessions, capped at 2/business and 5/individual, created only after Play Integrity/App Attest device attestation — see `server/services/deviceAuth.js`) or the `ApiKey` collection (business-only, admin-issued, recognizable by its `mdc_live_` prefix). Either resolves to `req.user`; `req.authMethod` is `"session"` or `"api_key"` accordingly. `verifyToken` rejects unauthenticated/inactive users; `getUserInfo` is the soft variant for routes that behave differently for logged-in vs anonymous callers; `requireSession` additionally gates endpoints (device management, push-token registration) that only make sense for a real device, not an API key. JWTs are still used, unrelated to this, for short-lived phone-verification codes and password-reset tokens.
...
- **Auth session expiry**: controlled server-side by `SESSION_EXPIRATION` (default 14d) vs `SESSION_REMEMBER_ME_EXPIRATION` (default 90d) in [server/config/authLimits.js](server/config/authLimits.js). Logging in now also requires device fields (`platform`, `deviceId`, `attestationToken`, a `challenge` fetched from `POST /device-challenge`) per [AUTH_SESSIONS_PLAN.md](AUTH_SESSIONS_PLAN.md) — the mobile-side Play Integrity/App Attest integration is not yet implemented (Phase 3), so login only succeeds today in sandbox mode, where attestation is faked.
